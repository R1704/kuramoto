...
- **KuramotoNCA:** the strongest long-term direction is a neural cellular automaton whose cells carry oscillatory state. The reference model is now `cell_i = (a_i, h_i, x_i)` with living matter, fixed hidden morphogen memory, and a unit oscillator. Phase 1 implements the `d = 2` scalar oscillator case, where `theta_i` is only a coordinate for `x_i = (cos theta_i, sin theta_i)`. Matter grows from Lenia-like density only when local living oscillator coherence supports it, and the four hidden channels now provide a minimal non-learned memory bridge before MLP training. Next learn small local coupling/perception kernels, then move to AKOrN-style vector oscillators on `S^{N-1}` after reproducible scalar experiments are solid.
