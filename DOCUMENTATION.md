...
Use `node scripts/dev-server.mjs` while editing. It serves the static app with `Cache-Control: no-store`, which prevents stale ES module graphs when checking changes in the browser. `python -m http.server` still works as a fallback, but may require a hard refresh or cache clear after code changes.
...
Our implementation extends the basic Kuramoto model with 8 coupling rules. Each captures different physical phenomena.
...
Rule 6 is **true Lenia on the living matter field** (rewritten 2026-06-10; the earlier
phase-density proxy `rho = 0.5 + 0.5 cos(theta)` could not sustain organisms by
construction, because growth fed back into phase velocity instead of a persistent
density). The update is:

```text
u_i = clamp( sum_j w_ij a_j / sum_{w_ij > 0} w_ij , 0, 1 )    # negative lobes subtract
G_i = growth_select(u_i, growthMu, growthSigma, growthMode)
a_i(t+dt) = clamp(a_i + dt * K0 * G_i, 0, 1)
```

Phase is untouched in rule 6 (`dtheta = 0`); the order buffer stores `a_i` so organism
detection tracks matter directly. `K0` acts as the Lenia update rate. The neighborhood
radius is `clamp(sigma2 * 3, 1, 12)` — `range` has no effect in rules 6/7.

The recommended kernel is **shape 7 (Lenia Shell)**: a nonnegative smooth ring with peak
at `1.5*sigma2` and thickness `max(0.3, sigma*0.5)`. The density minimum a localized
structure needs at its center comes from the ring geometry — Gaussian-core kernels
front-sweep into uniform fields instead.

Validated regime (2026-06-10, readback-verified at 256²): shell kernel with
`sigma=3.2, sigma2=3.6, growthMu=0.15, growthSigma=0.06, dt=0.1, K0=1` condenses seeds
into a stable lattice of ~600 discrete, persistent, high-contrast spots (each ~45 cells)
that the organism tracker identifies individually. Narrower growth windows
(`growthSigma <= 0.04`) collapse to extinction; the lattice colonizes the grid from any
seed — spatially confined single organisms (gliders, true Orbium) are a parameter-search
problem, which the (now fixed) Analysis sweep can target via `growthMu`.

Runtime controls:
- `growthMu`: center of growth response
- `growthSigma`: width of growth response
- `growthMode`: Gaussian / Step / Double-Gaussian
- `K0`: update rate; `sigma/sigma2`: kernel shape parameters
...
The loop is now **closed into the browser**. `sidecar/akorn-toy/export_weights.py` dumps the trained weights (the model is tiny), six demo scenes, and a verification reference to `src/akorn/akornModel.json`; `src/akorn/akornInference.js` is an exact JS port of the forward pass (convolutions, heads, unrolled dynamics) that runs live with no PyTorch. The **Analysis tab → 🧠 AKOrN panel** animates the relaxation on a chosen scene: you watch a near-uniform phase field organize into two clean phase domains over the foreground shapes, with the measured pairwise accuracy reported, and a Ground-truth toggle to compare. The JS port is proven bit-close to PyTorch (`scripts/verify-akorn-inference.mjs`: max abs error 1.9e-6 vs the baked reference; 100% segmentation on all six demo scenes). This is the trained counterpart to the Living Phase view: identity-by-phase, but learned rather than hand-tuned.

**Vector AKOrN in the viewer (2026-06-12).** The panel has a model selector: **Scalar S¹** (the 2-object phase segmenter above) and **Vector S¹⁵** — oscillators that are 16-dimensional unit vectors, trained on the 2× RTX 4090 server. The per-pixel 16-D vectors are projected to RGB via their top-3 PCA directions (`src/akorn/vectorInference.js`), so aligned vectors (same object) render as one colour; you watch 3+ overlapping shapes resolve into distinct colour regions at ~96% pairwise accuracy. The vector forward pass is again an exact JS port, verified bit-close to PyTorch (`scripts/verify-akorn-vector.mjs`, max abs error 4.3e-7). The reason for going vector is measured, not assumed: `sidecar/akorn-toy/RESULTS_nscaling.md` shows higher oscillator dimension extends the object-count limit (K=3 accuracy 0.77±0.09 for S¹ vs 0.95±0.01 for S¹⁵ over 3 seeds) — a circle crowds, a higher sphere holds more distinct identities. Workflow: trained and exported on the remote GPU box (`export_vector.py` → `akornVectorModel.json`), shipped to the local app over git, run in the browser with no PyTorch. Next research steps live in the sidecar — harder/larger scenes, energy readout, pushing n past 16.
...
Matter perception uses the existing Mexican-hat neighborhood split into support and surround terms:

Matter support is **phase-selective** (synchrony binding, 2026-06-10): a neighbor's
matter feeds growth only to the extent it is in phase with the receiving cell, while
inhibition stays phase-blind (competition for space is physical). `ncaPhaseAffinity`
mixes from 0 (phase-blind, the pre-binding model) to 1 (fully selective):

```text
aff_ij = 0.5 + 0.5 cos(theta_j - theta_i)
E_i = sum_{w_ij > 0} w_ij a_j mix(1, aff_ij, ncaPhaseAffinity) / sum_{w_ij > 0} w_ij
I_i = sum_{w_ij < 0} |w_ij| a_j / sum_{w_ij < 0} |w_ij|
u_i = clamp(E_i - beta I_i, 0, 1)
G_i = growth_select(u_i, growthMu, growthSigma, growthMode)
G_i+ = max(G_i, 0)
G_i- = max(-G_i, 0)
```

Oscillator perception is matter-normalized with a **living-mass floor** (2026-06-10): the
normalizer is `max(sum w_ij a_j, 0.05 * sum w_ij)`, so a near-empty neighborhood reads as
incoherent instead of a unit vector manufactured from one speck of matter divided by
epsilon:

```text
M_i = sum_{w_ij > 0} w_ij a_j z_j / max(sum_{w_ij > 0} w_ij a_j, 0.05 sum_{w_ij > 0} w_ij)
R_i = |M_i|
C_i = smoothstep(ncaCoherenceMin, ncaCoherenceMax, R_i)
T_i = Im(conj(z_i) M_i)
```
...
- If Rule 7 is selected manually, the UI switches to Matter and seeds a small central matter blob. Without nonzero matter, the NCA parameters have little visible effect because the rule is matter-weighted.
- Rule 7 now separates the Mexican-hat kernel into excitatory and inhibitory matter densities. `sigma` controls the growth-supporting inner lobe, `sigma2` controls the suppressive outer lobe, and `beta` controls how strongly outer matter inhibits local growth.
...
- **Lenia bestiary picker (2026-06-12).** The Dynamics tab has a searchable picker that spawns any of **463 organisms** from Bert Chan's reference Lenia bestiary (Orbiformes, Scutiformes, Echiniformes, Kroniformes, Radiiformes, Amoebiformes). This works because the app's growthMode 0 is exactly Lenia's exponential growth (`gn=1`) and a new **kernel shape 9** implements the exact multi-ring Lenia bell: `K(r) = b[floor(B·q)]·exp(4 − 1/(f(1−f)))` with `q = r/R`, `f = frac(B·q)`, `B` rings, ring intensities `b` read from the existing `ring_weight_1..5` uniforms. `spawnAnimal()` (src/patterns/leniaBestiary.js) decodes the published RLE, sets `R→sigma2`, `m→growthMu`, `s→growthSigma`, `dt = 1/T`, `b→ring weights`, and implants `count` copies. Coverage is the 95% of the bestiary using the exact bell core (`kn=1`) + exponential growth (`gn=1`) + ≤5 rings + radius ≤28 (the matter-rule neighborhood cap, raised to 30 for shapes 8/9); the dropped species use polynomial/step cores or radius up to 41. Bundle built by `scripts/build-lenia-bestiary.mjs` into `src/patterns/leniaBestiary.json` (RLE form, decoded in-browser). Verified by readback across all six orders: every spawned species is alive with mass conserved, gliders glide (Kronium solidus translates ~160 cells/6s), and rotators correctly spin in place (Hexadentium rotans, Gyrorbium gyrans displace <2 cells). The Rule 7 checkbox runs any bestiary organism under the coherence-gated oscillator model with a phase identity. Performance note: large-R species (24-28) run a ±28 neighborhood loop — drop grid size if FPS suffers.
- **Moving organisms (Migration presets, 2026-06-12).** A regime search found that the spot lattice marches coherently under an anisotropic kernel "wind": 60% shell ring + 40% directional Gaussian (kernel composition, secondary shape 3, asymmetry 1.0). All ~600 organisms stay confined and alive while translating in straight lines (rule 6: ~18 cells/12s at dt 0.1; rule 7: about half that). Honest framing: this is **driven drift, not self-propulsion** — the kernel breaks the symmetry and everything moves the same direction. Negative results from the same search, worth keeping: (a) an omega *gradient* does not transport rule 7 spots (settled-lattice displacement indistinguishable from control); (b) single crescent seeds at Orbium-like shell geometry have **no soliton window** — they die at growthSigma ≤ 0.018 and explode symmetrically at ≥ 0.020 (true Lenia gliders need the exact Orbium pattern, not crude seeds) — and this was then **confirmed constructively**: implanting the published Orbium unicaudatus pattern (decoded from the Lenia bestiary RLE) under the **exact Lenia bell kernel** (new kernel shape 8, `w(r) = exp(4 − 1/(q(1−q)))`, `q = r/R`, `R = sigma2 = 13`) at `growthMu 0.15, growthSigma 0.015, dt 0.1` yields genuine self-propelled gliders: three implanted copies translate rigidly in formation with mass exactly conserved (`🚀 Orbium (true glider)` preset). The lesson: Lenia self-propels through state-level asymmetry (the crescent shape regenerates itself displaced under an isotropic rule), which is different from rule-level asymmetry (the Migration wind) and from phase-pathway propulsion (impossible while binding is even in Δφ). **The marriage (`🎨 KuramotoNCA Orbium` preset):** the same gliders run under the FULL coherence-gated rule 7, each carrying a distinct phase identity visible as color in the Living Phase view. Getting there required making rule 7's matter update **Lenia-exact** — the earlier saturating factors (`birth·(1−a)`, `death·a`) flattened the growth response and blew Orbium up into a colony; the revised form `da = k·(gate²·G⁺ − G⁻ − decay·a)` with clamp bounds reduces to literal Lenia at gate=1/affinity=0/k=1/decay=0, so **ablation mode 2 is now a faithful pure-Lenia baseline** (it previously tested a warped variant). The validated spot/mitosis regimes survive the revision unchanged (re-verified by readback). Two measured facts about swimming identity: a moving organism constantly assimilates ambient cells, so (a) against a uniform ambient phase its identity *dilutes to the ambient* within seconds at ncaPhaseK 0.85 — the preset uses a random ambient and ncaPhaseK 4 so the body re-syncs newcomers faster than turnover; and (b) even then the identity is a **coherent random walk** (phaseR stays ~1.0, the three gliders stay mutually distinct, but each glider's phase wanders ~±2 rad over tens of seconds). Binding is kept at 0 for swimmers: a head plowing through phase-foreign ambient starves itself when support is phase-selective; (c) structurally, the binding term **cannot self-propel**: affinity = cos(Δφ) is even in the phase difference, so a smooth phase gradient pulls equally from both sides. A designed route to genuine per-organism self-propulsion is a Sakaguchi-style phase-lag in the binding term (`affinity = 0.5 + 0.5·cos(Δφ − χ)`, odd for χ ≠ 0), which makes organisms climb their own internal phase tilt. **This is now implemented and confirmed** (`ncaPhaseLag` χ, slot 57; `🧭 Phase-Lag Swimmer` preset). A matter blob carrying a frozen horizontal phase ramp under an *isotropic* kernel drifts along the ramp, driven only by the oscillator pathway — the first motion in the app caused by the Kuramoto half rather than the matter rule or kernel. The result is textbook for a Sakaguchi term, established by two independent zero-controls (readback, holding everything else fixed): (1) drift is **odd in χ** — χ=−1.2→+6.5, χ=0→0.0, χ=+1.2→−7.0 cells; (2) drift **requires a phase gradient** — at zero slope the blob does not move even at χ=0.8, and drift grows with the ramp slope (0.025→−3.6, 0.05→−10.6, 0.1→−12.4). Both the χ=0 and the zero-gradient controls give zero motion, so propulsion ∝ χ × phase-tilt and nothing else. This is the resolution of fault #2 for the Sakaguchi variant: the oscillator is now provably load-bearing — it produces directed motion that pure Lenia (even in Δφ) structurally cannot. Honest limits: the demo blob *blooms* as it swims (an isolated blob colonizes this Lenia regime), and in the mass-bounded spot-lattice regime the drift collapses to ≈0.2 cells because the lattice pins the spots (the same anchoring seen with the kernel wind). A mass-conserving phase-propelled *soliton* — a single organism that swims while holding its size — is the remaining tuning problem. Also discovered: in rule 7 the coherence gate and binding *anchor* organisms against drift (leading-edge cells read incoherent and phase-foreign), so the Migration preset loosens the gate (`ncaCoherenceMin 0.06 / Max 0.28`) and reduces affinity to 0.35.
...
- Keyboard shortcuts 0-7 for rule switching
...
| Key | Action | Category |
|-----|--------|----------|
| **0-7** | Switch coupling rule | Rules |
